...
$str = 'toggle 461,550 through 564,900
...
foreach($instruction_array as $instruction) {
  ...
  print_r($matches);
  power_lights($matches[1], $matches[2], $matches[3], $matches[4], $matches[5]);
}

function power_lights($instruction, $start_y, $start_x, $end_y, $end_x) {
  global $light_matrix;
  $lights_changed = 0;
  ...
  for($i=$start_y; $i<=$end_y; $i++) {
    for($j=$start_x; $j<=$end_x; $j++) {
      if($instruction == "toggle") {
        if(!isset($light_matrix[$i][$j]) || $light_matrix[$i][$j] === 0) {
          $power = 1;
        }
        else {
          $power = 0;
      ...
      $lights_changed++;
      $light_matrix[$i][$j] = $power;
    }
  }

  echo "Lights Changed: $lights_changed<br>";
}

$lights_lit = 0;
for($i=0; $i<=999; $i++) {
  for($j=0; $j<=999; $j++) {
    if(isset($light_matrix[$i][$j]) && $light_matrix[$i][$j] === 1)
      $lights_lit++;
  }
}
